...
export class SolarService {
    ...
    getDownloadFile(solar: Solar) {  console.log("testing:", solar["fileName"]);
        const headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': localStorage.getItem('token'), 'fileName':solar["fileName"],'folderId': solar["folderId"]});
        const options = new RequestOptions({ headers: headers })
        return this.http.get('https://www.solarsitedesign.com/webservicesangular/get_download_file',options).map(res => <any> JSON.parse(res['_body']));
    }
  ...
  getCreditCards() {
    ...
    return this.http.get('https://www.solarsitedesign.com/webservicesangular/get_credit_card',options).map(res => <any> JSON.parse(res['_body']));
  }

  updateCreditCard(id, card) {
    const headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': localStorage.getItem('token'), 'cardId': id });
    const options = new RequestOptions({ headers: headers, method: "put" })

    return this.http.put('https://www.solarsitedesign.com/webservicesangular/get_project_detail', card,options).map(res => <any> JSON.parse(res['_body']));
  }
