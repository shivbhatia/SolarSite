...
@Component({
  templateUrl: "./marketlisting.html",
})
...
export class marketlistingComponent implements OnInit{
    ...
    ngOnInit(){
        $('#mydiv').show();
        this.solarService.getMarkets(<Solar>this.model).subscribe(result => {
            this.result = result;
    ...
    delete_market(id:any) {
        this.confirmationService.confirm({
            message: 'Are you sure that you want to delete this inspection?',
            accept: () => {
                $('#mydiv').show();
                this.solarService.deleteMarket(<Solar>id).subscribe(result => {
                    this.result = result;
